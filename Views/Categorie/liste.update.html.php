...
<div class="flex flex-col md:flex-row justify-center  md:space-y-0  mt-20">
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <td class="px-6 py-4">
                            <a class="text-purple-600  dark:text-purple-400 hover:text-purple-800"
                                href="<?= WEBROOT ?>/?controller=categorie&action=update-categorie&id=<?= $categorie['idCategorie'] ?>">Modifier</a>
                            <a class="text-red-400 hover:text-red-900 ml-4"
                                href="<?= WEBROOT ?>/?controller=categorie&action=delete-categorie&id=<?= $categorie['idCategorie'] ?>"
    ...
    <div class="w-full mr-4 md:w-1/3   bg-gray-200 p-6 rounded-lg dark:bg-gray-600">
        <h2 class="text-2xl font-bold mb-6 mt-5 text-purple-900 dark:text-purple-400"">Modifier Categorie</h2>

        <form action="<?= WEBROOT ?>" method="post">
            <div class="mb-4">
                <label class="block text-gray-700 font-medium mb-2  dark:text-gray-200 ">Nom:</label>
                <input value="<?= $catSelect[0]['nomCategorie'] ?>" type="text" id="libelle" name="nomCategorie"
                    class="<?= add_classe_invalid('nomCategorie') ?> w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-purple-500 dark:bg-gray-800 dark:text-gray-400"
                    placeholder="nom du Categorie">
                <div class=" text-red-700 rounded relative  "
                    role="alert">
                    <strong class="font-bold "><?= $errors['nomCategorie'] ?? "" ?></strong>
                </div>
            </div>

            <div class="text-center">
                <input type="hidden" name="action" value="edit-categorie">
                <input type="hidden" name="id" value="<?= $catSelect[0]['idCategorie'] ?>">
                <input type="hidden" name="controller" value="categorie">

                <button id="btn" type="submit"
                    class="bg-purple-500 hover:bg-purple-600 text-white font-bold py-2 px-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-300 ">Modifier</button>
            </div>
        </form>
    </div>
