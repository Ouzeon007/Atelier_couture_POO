<div class="w-full  p-9">
    ...
    <form method="GET" action="<?= WEBROOT ?>" class="mb-4">
        <div class="flex space-x-6 justify-center">
            ...
            <div class="w-1/5">
                ...
                <input type="text" name="nom" value="<?= isset($_GET['nom']) ? $_GET['nom'] : '' ?>"
                    class="w-full px-3 py-2 border-b border-gray-300 rounded-lg focus:outline-none focus:border-purple-500 dark:bg-gray-800 text-gray-400">
            </div>
            ...
            <div class="mt-8">
                ...
                <input type="hidden" name="action" value="filter-vente">
...
<div>
    <table
        ...
        <caption
            ...
            <a class="rounded-md font-bold bg-purple-500 hover:bg-purple-600 focus:ring-4 ring-purple-300 shadow-sm px-2 py-2 text-white focus:outline-2 outline-transparent outline-offset-4 float-end active:bg-purple-600/90 ml-3"
                href="<?= WEBROOT ?>/?controller=vente&action=form-vente">
        ...
        <tbody>
            <?php foreach ($reponse["data"] as $vente):
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <td class="px-6 py-4">
                        ...
                        <a class="text-purple-600 dark:text-purple-400 hover:text-purple-800 "
                            href="<?= WEBROOT ?>/?controller=appro&action=update-article&id=<?= $vente['idVente'] ?>">Modifier</a>
                        <a class="text-red-400 hover:text-red-900 ml-4"
                            href="<?= WEBROOT ?>/?controller=appro&action=delete-article&id=<?= $vente['idVente'] ?>"
    ...
    <nav aria-label="Page navigation example">
        <ul class="flex items-center -space-x-px h-10 text-base mt-5  justify-center">
            <li>
                <a href="<?= WEBROOT ?>/<?= WEBROOT ?>/?date=<?php if (isset($_GET['date']))
                        echo $_GET['date'] ?><?php if (isset($_GET['articleId']))
                        echo "&articleId=" . $_GET['articleId'] ?>&nom=<?php if (isset($_GET['nom']))
                        echo $_GET['nom'] ?>&controller=vente&action=filter-vente&page=<?php if ($currentPage > 0)
                        echo $currentPage - 1;
                    else
                        echo 0 ?>"
            ...
            <?php for ($page = 0; $page < $reponse["pages"]; $page++): ?>
                <li>
                    <a href="<?= WEBROOT ?>/?date=<?php if (isset($_GET['date']))
                          echo $_GET['date'] ?><?php if (isset($_GET['articleId']))
                          echo "&articleId=" . $_GET['articleId'] ?>&nom=<?php if (isset($_GET['nom']))
                          echo $_GET['nom'] ?>&controller=vente&action=filter-vente&page=<?= $page ?>"
            ...
            <li>
                <a href="<?= WEBROOT ?>/<?= WEBROOT ?>/?date=<?php if (isset($_GET['date']))
                        echo $_GET['date'] ?><?php if (isset($_GET['articleId']))
                        echo "&articleId=" . $_GET['articleId'] ?>&nom=<?php if (isset($_GET['nom']))
                        echo $_GET['nom'] ?>&controller=vente&action=filter-vente&page=<?php if ($currentPage < $reponse["pages"] - 1)
                        echo $currentPage + 1;
                    else
                        echo $_GET["page"] ?>"
